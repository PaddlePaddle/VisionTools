...
class KVConfHelper {
public:
    KVConfHelper (const kv_conf_t &conf)
        : _conf(conf) {
    }
    ...
    bool get(const std::string &k, int &v, int def = -1) const {
        ...
        try {
            ...
            std::string err_msg = string_format("parse conf failed:with errmsg[%s]"\
                                    " and input:[k:%s,v:%s]", e.what(), k.c_str(), value.c_str());
            LOG(WARNING) << err_msg;
            return false;
        }
    }
    ...
    bool get(const std::string &k, std::vector<float> &v,
            std::string sep = ",") const {
        std::string value = get(k);

        v.clear();
        if (value.size() > 0) {
            for (auto &i : string_split(value, sep)) {
    ...
    bool get(const std::string &k, std::vector<int> &v,
            std::string sep = ",") const {
        std::vector<float> fv;
...
static int process_resize(const KVConfHelper &conf, cv::Mat &input,
        cv::Mat &result, std::string &errmsg, BufLogger &logger) {
    int ret = 0;
    ...
    if (conf.get("short_size", short_size)) {
        float percent = float(short_size) / std::min(img_w, img_h);
        ...
        errmsg = string_format("not found valid 'resize_w' or 'resize_h' param");
        return ret;
    }
    logger.append("[resize:{w:%d,h:%d}]", resize_w, resize_h);

    cv::Size size(resize_w, resize_h);
    ret = resize_image(input, size, result, interpo);
    if (ret || result.empty()) {
...
static int process_crop(const KVConfHelper &conf, cv::Mat &input,
        cv::Mat &result, std::string &errmsg, BufLogger &logger) {
    int ret = 0;
    int crop_x = 0;
    int crop_y = 0;
    ...
    int img_h = input.rows;
    int img_w = input.cols;
    if (conf.get("crop_center", crop_center) 
            && conf.get("crop_w", crop_w)
        ...
        if (crop_center) {
            ...
            crop_x = int_rand_range(0, img_w - crop_w);
            crop_y = int_rand_range(0, img_h - crop_h);
        }
    } else if (!conf.get("crop_x", crop_x)
            || !conf.get("crop_y", crop_y)
            || !conf.get("crop_w", crop_w)
            || !conf.get("crop_h", crop_h)) {
    ...
    logger.append("[crop:{x:%d,y:%d,w:%d,h:%d}]", crop_x, crop_y, crop_w, crop_h);
    cv::Rect rect(crop_x, crop_y, crop_w, crop_h);
    ret = crop_image(input, rect, result);
    if (ret || result.empty()) {
...
static int process_random_crop(const KVConfHelper &conf, cv::Mat &input,
        cv::Mat &result, std::string &errmsg, BufLogger &logger) {
    int img_h = input.rows;
    int img_w = input.cols;
    int ret = 0;
    ...
    if (!conf.get("final_size", final_size) \
            || final_size.size() != 2 \
    ...
    float aspect_ratio = sqrt(float_rand_range(ratio[0], ratio[1]));
    float w = 1.0 * aspect_ratio;
    float h = 1.0 / aspect_ratio;

    float bound = std::min((float(img_w) / img_h) / pow(w, 2),
            ((float(img_h) / img_w)) / pow(h, 2));

    float scale_max = std::min(scale[1], bound);
    float scale_min = std::min(scale[0], bound);
    float target_area = img_w * img_h * float_rand_range(scale_min, scale_max);
    float target_size = sqrt(target_area);

    w = floor(target_size * w);
    h = floor(target_size * h);

    int i = int_rand_range(0, int(img_w - w));
    int j = int_rand_range(0, int(img_h - h));

    logger.append("[crop_rect:{x:%d,y:%d,w:%d,h:%d}", i, j, int(w), int(h));
    cv::Rect rect(i, j, int(w), int(h));
    cv::Mat cropped;
    ret = crop_image(input, rect, cropped);
    if (ret || cropped.empty()) {
        errmsg = string_format("rand_crop.crop image failed with ret[%d]", ret);
        ret = TRANS_ERR_RAND_CROP_INVALID_PARAM;
        return ret;
    }
    ...
    ret = resize_image(cropped, size, result, interpo);
    if (ret || result.empty()) {
        errmsg = string_format("rand_crop.resize image failed with ret[%d]", ret);
        ret = TRANS_ERR_RAND_CROP_INVALID_PARAM;
        return ret;
    }
...
static int process_rotate(const KVConfHelper &conf, cv::Mat &input,
        cv::Mat &result, std::string &errmsg, BufLogger &logger) {
    int ret = 0;
    int angle = 0;
    int random_range = 0;
    ...
    if (conf.get("random_range", random_range)) {
        angle = int_rand_range(-random_range, random_range);
        logger.append("[random_range:%d]", random_range);
    } else if (!conf.get("angle", angle)) {
        errmsg = string_format("rotate op not found valid 'random_range' param");
    ...
    ret = rotate_image(input, float(angle), result, resample);
    if (ret || result.empty()) {
...
static int process_flip(const KVConfHelper &conf, cv::Mat &input,
        cv::Mat &result, std::string &errmsg, BufLogger &logger) {
    int ret = 0;
    int flip_code = 0;
    int random = 0;
    ...
    if (random && !int_rand_range(0, 1)) {//no need to flip
        result = input;
        return ret;
    }

    logger.append("[random:%d, flip_code:%d]", random, flip_code);
    ret = flip_image(input, flip_code, result);
    if (ret || result.empty()) {
        errmsg = string_format("failed to resize image with ret[%d]", ret);
        ret = TRANS_ERR_FLIP_INVALID_PARAM;
        return ret;
...
typedef int processor_t(const KVConfHelper &conf, cv::Mat &input,
        cv::Mat &result, std::string &errmsg, BufLogger &logger);
...
ImageTransformer::~ImageTransformer() {
    ...
    while (!this->_output_queue.is_empty()) {
        ...
        if (d) {
            LOG(INFO) << "delete unconsumed data[" << d->id
                << "] in output queue";
            delete d;
        } else {
...
int ImageTransformer::add_op(const std::string &op_name, const kv_conf_t &conf) {
...
bool ImageTransformer::is_stopped() {
...
static void swapaxis(cv::Mat & mat, std::string & outstr){
    if (mat.channels() == 1){
        std::memcpy(&outstr[0], mat.data, mat.total() * mat.elemSize());
    } else {
        std::vector<cv::Mat> channels(mat.channels());
        int channelimgsize = mat.rows * mat.cols;
        cv::split(mat, channels);
        for (size_t i = 0; i < channels.size(); i++) {
            std::memcpy((char *)&outstr[0] + i * channelimgsize, 
                    channels[i].data, channels[i].total() * channels[i].elemSize());
        }
    }
}
void ImageTransformer::process(const transformer_input_data_t &input) {
    ...
    try {
        for (size_t i = 0; i < _ops.size(); i++) {
            ...
            logger.append("ret:%d,elenum:%d,cost:%lums]}",
                    err_no, result.total(), op_cost);
            if (err_no || result.empty()) {
                LOG(WARNING) << string_format("failed to execute op[%s] with err_no[%d]",
                        op_name.c_str(), err_no);
                if (!err_no) {
    ...
    logger.append("[output:{size:%d,err_no:%d,err_msg:[%s]}]",
            output->data.size(), err_no, err_msg.c_str());
