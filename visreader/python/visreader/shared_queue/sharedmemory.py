...
class PageAllocator(object):
    ...
    def malloc_page(self, page_num):
        ...
        if page_status != '0' * page_num:
            ...
            if free_pages == 0:
                ...
                err_msg = 'not found available pages with page_status[%s] '\
                    'and %d free pages' % (page_status, free_pages)
            err_msg = 'failed to malloc pages for [%s]' % (err_msg)
            logger.warn(err_msg)
        ...
        assert self.get_page_status(pos, page_num) == '1' * page_num, \
